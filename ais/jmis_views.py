...
class LostReportSerializer(serializers.ModelSerializer):
    ...
    mv_imo = serializers.CharField(write_only=True, required=False, allow_null=True, allow_blank=True)
    mv_ship_name = serializers.CharField(write_only=True, required=False, allow_null=True, allow_blank=True)
    mv_flag = serializers.CharField(write_only=True, required=False, allow_null=True, allow_blank=True)
    mv_ais_type_summary = serializers.CharField(write_only=True, required=False, allow_null=True, allow_blank=True)
        ...
        try:
            ...
            for row in rows:
                ...
                converted_data.append({
                    ...
                    "lr_remarks": row.get("Remarks"),
                    "lr_created_on": row.get("CreatedOn"),
                    "lr_created_by": row.get("CreatedBy"),
                    "lr_track_status": row.get("TrackStatus"),
                    "lr_total_crew": row.get("TotalCrew") if row.get("TotalCrew") in row else None,
                })
...
class SitReportSerializer(serializers.ModelSerializer):
    ...
    mv_imo = serializers.CharField(write_only=True, required=False, allow_null=True, allow_blank=True)
    mv_mmsi = serializers.CharField(write_only=True, required=False, allow_null=True, allow_blank=True)
    mv_ship_name = serializers.CharField(write_only=True, required=False, allow_null=True, allow_blank=True)
    mv_flag = serializers.CharField(write_only=True, required=False, allow_null=True, allow_blank=True)
    mv_ais_type_summary = serializers.CharField(write_only=True, required=False, allow_null=True, allow_blank=True)
        ...
        try:
            ...
            for row in rows:
                ...
                converted_data.append({
                    ...
                    "ps_status_symbol": row.get("StatusSymbol"),
                    "ps_status_symbol_remarks": row.get("StatusSymbolRemarks"),
                    "ps_status_symbol_assigned_time": row.get("StatusSymbolAssignedTime"),
                    "ps_track_number": row.get("TrackNumber"),
                    "ps_position": {
                        "type": "Point",
                        "coordinates": [float(long), float(lat)]
                    },
                    "ps_speed": row.get("Speed"),
                    "ps_course": row.get("Course"),
                    "ps_timestamp": row.get("Timestamp"),
                    "ps_lastport": row.get("LastPort"),
                    "ps_next_port": row.get("NextPortName"),
                    "ps_track_type": row.get("TrackType"),
                    "ps_track_label": row.get("TrackLabel")
