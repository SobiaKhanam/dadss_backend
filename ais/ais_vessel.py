...
class MerchantSerializer(CustomSrSerializer):
    ...
    child = MerSreports2
    msr_type = serializers.HiddenField(default='MERCHANT')
...
class CustomSrViewSet(CustomViewSet):
    filterset_fields = ['msr_key', 'msr_pf_id', 'msr_mv_key', 'msr_dtg', 'msr_type', 'msr_movement', 'msr_action']
    ordering = ['-msr_key']
...
class MerchantViewSet(CustomSrViewSet):
    queryset = MerSreports.objects.filter(msr_type='MERCHANT')
